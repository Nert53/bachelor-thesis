...
Vizuální část aplikace je z~pohledu uživatelského rozhraní velmi důležitá. Řadí se od něj barvy v aplikaci, ikona aplikace, použité fonty atd. Primární barvou aplikace jsem vzolil tmavě tyrkysovou. Je nastavená jako výchozí barva v barevném tématu a použil jsem ji i pro ikonu aplikace. Ikona aplikace byla vytvořena v nástroji Figma a byla v ní použity ikony z Google Fonts (viz obrázek \ref{fig:appicon}). Jako font aplikace jsme zvolil \textit{Poppins}, který je podobný doporučenému fontu \textit{Roboto} ze systému Material Design. Fonty jsou vidět na obrázku \ref{fig:font}. 

\begin{figure}
  \centering
  \includegraphics[width=0.4\textwidth]{images/app-icon.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=0.48\textwidth]{images/poppins.png}
  \hfill{}
  \includegraphics[width=0.48\textwidth]{images/roboto.png}
  \caption{Font Poppins (vlevo) a Roboto (vpravo)}
  \label{fig:font}
\end{figure}
...
Úvodní obrazovka je centrem dění pro uživatele. Poskytuje základní přehled o~finanční situaci. Konkrétně zobrazuje graf útraty podle kategorií (v~aktuálním měsíci), stav účtu, dnešní útratu a posledních pět transakcí (viz ). Na zařízení s~větší obrazovkou je rozšířena o~legendu grafu a další zajímavé číselné údaje o~stavu účtu. Úvodní obrazovka není uživatelsky přizpůsobitelná.
...
Poslední obrazovkou dostupnou z~navigační lišty je nastavení (záložka \textit{Settings}). Umožňuje nastavit jak prostředí po stránce funkcí, tak i po stránce vzhledu. Taktéž zde uživatel nalezne informace o~aplikaci a možnost smazání všech dat.
